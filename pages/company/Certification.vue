  ...
  <view class="page-base">
    <template v-if="type == 1">
      ...
      <view class="card">
        ...
        <view class="up-box flex-ct form-box" @click="pickImg">
          <image v-if="form.license" :src="form.license | imgFilter" mode="aspectFit"></image>
    ...
    <template v-if="type == 3">
      <view class="success-box flex-col">
        ...
        <navigator hover-class="none" url="/pages/tabbar/home?tab=2" class="btn-primary">
          {{ jumpSeconds }}S   发起签署
        </navigator>
      </view>
    </template>
...
export default {
  ...
  onShow() {
    ...
    // 	uni.redirectTo({
    // 		url: '/pages/company/authorize?path='+ encodeURIComponent(getCache('eCerCompenytURL'))
    // 	})
    // }else{
    // 	that.showPage = true
    // }
              ...
              success: function (image) {
                if (image.type == 'png' || image.type == 'jpeg' || image.type == 'jpg') {
                  upload(res.tempFiles[0])
                    .then(path => {
                      if (path) {
                        that.license.license = path;
                        ocr(that.license)
                          .then(ocrBack => {
                            if (ocrBack) that.form = ocrBack;
                            uni.hideLoading();
                          })
                          .catch(() => {
                            uni.hideLoading();
